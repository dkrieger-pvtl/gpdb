...
# The @concourse_cluster tag denotes the scenario that requires a remote cluster
    ...
    Scenario: gpinitsystem creates a cluster using -I configuration file
        Given a working directory of the test as '/tmp/gpinitsystem_config'
        And the database is not running
        And generate an input cluster config file '/tmp/gpinitsystem_config/clusterConfigfile'
        When initialize a cluster using '/tmp/gpinitsystem_config/clusterConfigfile'
        Then gpinitsystem should return a return code of 0
