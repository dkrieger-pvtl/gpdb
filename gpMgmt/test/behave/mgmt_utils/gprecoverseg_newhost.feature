...
# The @concourse_cluster tag denotes the scenario that requires a remote cluster
    ...
    Scenario: gprecoverseg -p successfully fails over to a new host
      ...
      And host "sdw1" is disconnected from the cluster
      And the user runs psql with "-c 'SELECT gp_request_fts_probe_scan()'" against database "postgres"
      And the user runs psql with "-c 'BEGIN; CREATE TEMP TABLE tempt(a int); COMMIT'" against database "postgres"
      When the user runs "gprecoverseg -a -p sdw4"
      Then gprecoverseg should return a return code of 0
      And the cluster configuration is saved "after"
      And the "before" and "after" cluster configuration matches with the expected for gprecoverseg newhost
