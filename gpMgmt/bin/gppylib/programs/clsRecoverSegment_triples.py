...
# Revisit: gparray is mutated for all triplets, even if we skip recovery for them(if they are unreachable)
...
class RecoveryTriplets(abc.ABC):
    ...
    # REVISIT: the returned RecoveryTriplet(s) reflect (failed, live, failover) with failover reflecting the recovery
    ...
    def _convert_requests_to_triplets(self, requests: List[RecoveryRequest]) -> List[RecoveryTriplet]:
        triplets = []
...
class RecoveryTripletsInplace(RecoveryTriplets):
    ...
    def getTriplets(self):
        ...
        # REVISIT: only get failed mirrors explicitly here? gp_segment_configuration should
